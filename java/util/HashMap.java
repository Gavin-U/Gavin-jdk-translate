...
/**
 *  1、首先明白这个&符号这块它是一个二进制运算
  *     比如 0000 1111  和这个 0101 0101 进行这个&运算得出的结果就是 0000 0101  它是一个如果一样就等于1不同就为0
  *     他计算这个数组的下标的时候，按照我们的想法会把他的hash值进行取模然后计算出下标这样可以实现这个index，但是这块用到了这个&运算二进制运算效率这方面会比取模好的多
  *     这个hashMap 在&运算时  
 *       
 */
public class HashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
	...
	/*
	*	jdk8 为什么加了这个红黑树
	* 	 1、我们首先拿链表、完全平衡二叉树、和这个红黑树来比较
	* 	 	（1）、链表： 插入快，查询慢、
	* 	 	（2）、完全平衡二叉树：它其实主要浪费性能的地方主要是这个平衡这里
	* 	 	（3）、红黑树：因为他不需要平衡 然后查询增加也比链表和完全平衡二叉树性能要快、所以用了红黑树
	* 	 		Ps-- 因为这个hashMap需要get。put都需要性能所以引进了这个红黑树
	*
	* */
	...
	/**
	 * 这是hashMap初始化的一个容量 默认是16
	 */
	static final int DEFAULT_INITIAL_CAPACITY = 1 << 4; // aka 16

	/**
	 * 这是HashMap最大容量是2的30次方
	 */
	static final int MAXIMUM_CAPACITY = 1 << 30;

	/**
	 * 这是一个他的默认加载因子大小,这里可以传参进行修改
	 */
	static final float DEFAULT_LOAD_FACTOR = 0.75f;

	/**
	 *	数组上的链表如果大于这个8就会把这个链表变成红黑树
	 */
	static final int TREEIFY_THRESHOLD = 8;

	/**
	 * 数组上的链表如果小于这个6就会把这个红黑树变成链表
	 */
		...
		if ((p = tab[i = (n - 1) & hash]) == null)
			...
			//这里他判断你当前这个节点(就是数组上面的元素)是不是一颗树
				...
				for (int binCount = 0;; ++binCount) {
					//判断是不是最后一个节点
					if ((e = p.next) == null) {
						//如果是那它就会直接newNode赋值给p.next
						//因为你当前已经是最后一个节点了
						//所以就相当于把现在的元素加在这个链表的尾部
						/* 这里添加到尾部就解决了1.7的这个多线程扩容时的这个死循环问题 */
	...
	/**
	 * resize() 这个方法主要的这个思路主要是围绕着这个二进制来计算的
	 * 因为你的每一次扩容是 *2   她走的二进制其实就是   000<p 其实他判断的就是这个位上的改动>1</p> 0000
	 * 当然我这说的可能比较笼统
	 */
	...
	abstract class HashIterator {
		...
		final Node<K, V> nextNode() {
			Node<K, V>[] t;
			Node<K, V> e = next;
			//这里在多线程修改时  因为HashMap在put,remove,会对这个modCount++,
			// 所以在多线程下导致这两个值不相等、会报这个错
			// HashMap用到了这块  其实也解决了线程安全问题. 也就是多线程下如果修改这个HashMap
			// 然后造成了这个脏读 所以HashMap用这个报错来解决
/*
			  这里就是fail-fast快速容错机制
*/
